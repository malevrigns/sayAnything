...
class _VideoAttachmentTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Semantics(
    button: true,
    label: '播放视频 ${attachment['name'] ?? ''}'.trim(),
    child: Material(
      color: Colors.white.withValues(alpha: .055),
      borderRadius: BorderRadius.circular(17),
      clipBehavior: Clip.antiAlias,
      child: InkWell(
        onTap: () => Navigator.push<void>(
          ...
          MaterialPageRoute(
            builder: (_) => _VideoViewerPage(
              title: attachment['name']?.toString() ?? '视频',
              loadController: ({required bool refresh}) async {
                final url = await api.mediaUrl(
                  '${attachment['id']}',
                  refresh: refresh,
                );
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.play, size: 18),
              ),
              const SizedBox(height: 9),
              Text(
                attachment['name']?.toString() ?? '视频',
                maxLines: 1,
