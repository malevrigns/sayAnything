...
## 1.4.1 四栏导航与统一玻璃选项（2026-09-11）

- 主导航调整为「动态 / 聊天 / 附近 / 我」，附近作为一级页面不显示返回箭头；我包含匿名资料、发布与收藏。动态页左上角 sayAnything 标识是可操作的设置按钮，进入设置及其他子页面时隐藏底栏。
- 性别弹窗使用透明 Dialog 与 32px 圆角强玻璃，性别/字号共用 GlassChoiceBar，选项横向排列并通过圆角高光表达选中。按钮保留至少 44px 的触控区域。
- Flutter 静态检查、22 项测试通过，包含 360px 四导航/设置返回及附近不自动发布位置回归。品牌被 AppBar 当作标题的问题已修复，浏览器中具有按钮语义。
- Glass、媒体、附近三组端到端验证通过。最终复核 360 / 390 / 430 / 1440px 的导航、设置、字号、个人页和性别弹窗截图。截图仅位于本机 artifacts，测试输入与悬停提示时序已稳定处理。
- 四个 APK 与 Windows 包构建成功，均为 1.4.1（6），原有架构、签名、权限和文字字体保持。APK 签名与 16 KiB ZIP 对齐通过；五个客户端包的 HTTP 下载大小及 SHA-256 与发行清单一致。

此版为客户端界面调整，Go 1.4 后端保持兼容。网页预览已经更新，未自动更新用户手机；手机此前已在用户确认后成功安装并启动 1.4。
