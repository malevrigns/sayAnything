...
1.4 版新增真实视频封面、主动开启的同校附近列表和打招呼，资料只新增可选性别。业务规则、位置范围和招呼限流由 Go 负责。主导航仍为「聊天 / 记录 / 设置」，视频、图片、广场、房间和私聊保持灰度玻璃界面。[前后端职责说明](docs/architecture.md)。
...
| 记录 | 按日期分组的私聊、未读数、Go 检索昵称及历史消息、每 3 秒更新当前对话 |
| 附近的人 | 聊天页右上方入口；主动开启大致定位，同校约 5 公里内，模糊距离、自动到期、关闭展示、一次性招呼进入私聊 |
...
在设置中选择「男 / 女 / 不透露」，默认不透露，不新增年龄、实名或头像资料。附近展示默认关闭，只有主动开启后才获取设备大致位置。Go 将位置按 0.01 度网格取整保存，约 30 分钟后自动隐藏；不返回坐标或精确距离，只显示距离范围，也不后台追踪位置。关闭展示会清除当前位置，已有私聊保留。
