...
1.3 版将分类、输入限制和聊天记录搜索集中到 Go，安卓增加更小的架构专用安装包。帖子、评论、话题房间和私聊继续支持图片与视频，沿用校园匿名真人交流和全灰度液态玻璃 UI。主导航为「聊天 / 记录 / 设置」。动态背景加载失败或开启减少动态效果时使用静态灰色渐变。[前后端职责说明](docs/architecture.md)。
...
- `sayanything-android-arm64.apk`：ARM64 版，31.06 MiB，官网优先提供；另有 `-armv7.apk`（32 位）及 `-x64.apk`（模拟器）。不确定架构时可用通用版。
...
更新到 1.3 时请先升级 Go 后端，再安装客户端；新客户端需要后端的 `/api/v1/config` 接口。旧客户端可继续连接新版 Go。
